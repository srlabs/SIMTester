...
public class FileManagement {
    ...
    public static SimCardFile selectPath(String filePath) throws CardException, FileNotFoundException {
        ...
        for (int i = 0; i < filePath_bytes.length; i += 2) {
            ...
            if ((byte) 0x9f == (byte) r.getSW1()) {
                SelectResponse selectResponse = FileManagement.getResponse(fileId, r.getSW2());

                if (DEBUG) {
                    System.out.println(LoggingUtils.formatDebugMessage("response: " + HexToolkit.toString(selectResponse.getResponseData())));
                }
                ...
                switch ((byte) selectResponse.getFileType()) { // fileType byte
                    ...
                    case SimCardFile.DF:
                        ...
                        break;
                    case SimCardFile.EF:
                        switch ((byte) selectResponse.getResponseData()[13]) { // fileStructure byte
    ...
    public static SelectResponse getResponse(byte[] fileId, int bytes) throws CardException {
        ResponseAPDU response = APDUToolkit.getResponse(bytes);

        SelectResponse fileData = null;
        ...
        if ((short) 0x9000 == (short) response.getSW()) {
            ...
            fileData = new SelectResponse(response.getData(), HexToolkit.toString(fileId));
